...
NHANES_23_t4a = subset(NHANES_23, IND310<=5 & HIQ011 <=5)

library(ggplot2)
ggplot(NHANES_23_t4a, aes(HIQ011, INDFMMPI, group=HIQ011)) + 
  geom_boxplot() +
  theme_gray()

'| A ampla maioria dos que não têm plano de saúde possuem índice de pobreza entre 1 e 2.'

ggplot(NHANES_23_t4a, aes(HIQ011, IND310, group=HIQ011)) + 
  geom_boxplot() +
  theme_gray()
...
NHANES_23_t4b = subset(NHANES_20, IND310<=5 & HUQ030<=5)

ggplot(NHANES_23_t4b, aes(IND310, fill=as.factor(HUQ030))) + 
...
NHANES_23_t4c = subset(NHANES_20, IND310<=5 & HUQ042<=6)

library(dplyr)
NHANES_23_t4c = NHANES_23_t4c %>%
...
ggplot(NHANES_23_t4c, aes(reorder(IND310_txt, IND310), fill=HUQ042_txt)) + 
  geom_bar(aes(y = after_stat(count / tapply(count, x, sum)[x])),
           position = "dodge") +
  scale_y_continuous(labels = scales::percent) +
  labs(y = "Percentage of people") +
...
NHANES_23_t4d = subset(NHANES_20, IND310<=5 & HUQ055<=5)

NHANES_23_t4d = NHANES_23_t4d %>%
  mutate(IND310_txt = case_when(
    IND310 == "1" ~ "< 3K",
    IND310 == "2" ~ "3K-5K",
    IND310 == "3" ~ "5K-10K",
...
ggplot(NHANES_23_t4d, aes(reorder(IND310_txt, IND310), fill=HUQ055_txt)) + 
  geom_bar(aes(y = after_stat(count / tapply(count, x, sum)[x])),
           position = "dodge") +
  scale_y_continuous(labels = scales::percent) +
  labs(y = "Percentage of people") +
...
NHANES_23_t4e = subset(NHANES_20, IND310<=5 & HUQ090<=5)

NHANES_23_t4e = NHANES_23_t4e %>%
  mutate(IND310_txt = case_when(
    IND310 == "1" ~ "< 3K",
    IND310 == "2" ~ "3K-5K",
    IND310 == "3" ~ "5K-10K",
...
ggplot(NHANES_23_t4e, aes(reorder(IND310_txt, IND310), fill=HUQ090_txt)) + 
  geom_bar(aes(y = after_stat(count / tapply(count, x, sum)[x])),
           position = "dodge") +
  scale_y_continuous(labels = scales::percent) +
  labs(y = "Percentage of people") +
