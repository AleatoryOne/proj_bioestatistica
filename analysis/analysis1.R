...
library(ggplot2)
ggplot(subset(NHANES_12, !is.na(HealthGen)), aes(HealthGen, Poverty, fill=HealthGen)) + 
  geom_boxplot() +
  theme_gray() +
...
library(car)
leveneTest(Poverty ~ HealthGen, data = NHANES_12)

'| O teste de Levene para homogeneidade das variâncias mostra distribuição não homogênea.'

oneway.test(Poverty ~ HealthGen, data=NHANES_12, var.equal = FALSE)
