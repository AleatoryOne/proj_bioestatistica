...
ggplot(NHANES, aes(Race1, HHIncomeMid, fill=Gender)) + 
  geom_boxplot() +
  theme_gray() +
  scale_fill_brewer(palette="YlOrBr") +
  stat_summary(fun = median, geom = "line", aes(group=Gender, color=Race1))
...
ggplot(NHANES, aes(Gender, HHIncomeMid, fill=MaritalStatus)) + 
  geom_boxplot() +
  theme_gray() +
  scale_fill_brewer(palette="YlOrBr") +
  stat_summary(fun = median, geom = "line", aes(group=Gender, color=Race1))
...
ggplot(NHANES, aes(sample=HHIncomeMid))+
  geom_qq() +
  geom_qq_line() + 
  facet_grid(~Gender)
...
byf.shapiro(HHIncomeMid ~ Gender, NHANES)
leveneTest(HHIncomeMid ~ Gender, NHANES)#, center=mean)

t.test(HHIncomeMid ~ Gender, NHANES, var.equal=TRUE)
